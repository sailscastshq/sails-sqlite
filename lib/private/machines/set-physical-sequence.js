module.exports = {
  ...
  fn: function (inputs, exits) {
    ...
    // PostgreSQL-style sequences are named like 'users_id_seq', 'invoice_items_id_seq', etc.
    // The table name is everything before '_id_seq'
    let tableName = sequenceName

    // Handle PostgreSQL-style sequence names (e.g., 'invoice_items_id_seq' -> 'invoice_items')
    if (sequenceName.endsWith('_id_seq')) {
      tableName = sequenceName.slice(0, -7) // Remove '_id_seq' suffix
    } else if (sequenceName.endsWith('_seq')) {
      tableName = sequenceName.slice(0, -4) // Remove '_seq' suffix
    }
