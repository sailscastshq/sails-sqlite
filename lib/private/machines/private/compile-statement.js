...
module.exports = function compileStatement(statement) {
  ...
  if (statement.unionAll && Array.isArray(statement.unionAll)) {
    ...
    // Check if all union statements have the same ORDER BY, LIMIT, and SKIP
    let hasConsistentModifiers = true
    let firstLimit = null
    let firstSkip = null
    let firstOrderBy = null

    statement.unionAll.forEach((unionStatement, index) => {
      if (index === 0) {
        firstOrderBy = unionStatement.orderBy
        firstLimit = unionStatement.limit
        firstSkip = unionStatement.skip
      } else {
        // Check if this statement has the same modifiers as the first
        if (
          JSON.stringify(unionStatement.orderBy) !==
            JSON.stringify(firstOrderBy) ||
          unionStatement.limit !== firstLimit ||
          unionStatement.skip !== firstSkip
        ) {
          hasConsistentModifiers = false
        }
      }
    })
    ...
    statement.unionAll.forEach((unionStatement) => {
      ...
      // If modifiers are consistent across all statements, remove them and apply globally
      if (hasConsistentModifiers && firstOrderBy) {
        globalOrderBy = firstOrderBy
    ...
    // Apply global ORDER BY only if modifiers were consistent
    if (
      hasConsistentModifiers &&
      globalOrderBy &&
    ...
    // Apply global LIMIT only if modifiers were consistent
    if (hasConsistentModifiers && typeof firstLimit === 'number') {
      sql += ` LIMIT ${firstLimit}`
    }

    // Apply global OFFSET only if modifiers were consistent
    if (hasConsistentModifiers && typeof firstSkip === 'number') {
      sql += ` OFFSET ${firstSkip}`
    }
